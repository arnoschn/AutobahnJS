...
Log.prototype._open_group = function(close_children) {
    close_children = close_children==undefined?true:close_children;
    if(this._group_is_open || !this._group) {
        return;
    }
    close_children && this._close_children();
    if(this._parent && !this._parent._group_is_open) {
        this._parent._open_group(false);
    } else if(!this._parent) {
        if(Log._open_parent !== null && Log._open_parent!== this._id) {
            var open_log = Log._parents[Log._open_parent];
            open_log.close();
            Log._open_parent =this._id;
